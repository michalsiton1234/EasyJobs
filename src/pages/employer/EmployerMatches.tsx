...
import { useNavigate } from "react-router-dom";
import { Zap, Send, Sparkles, LayoutDashboard, Loader2 } from "lucide-react";
import { useEmployerMatches } from "@/hooks/useEmployerMatches";
import MatchCard from "@/components1/employer/MatchCard";
import JobSelectionModal from "@/components1/employer/JobSelectionModal";
import "@/style/employer/EmployerMatches.css";
        ...
        <div className="matches-grid">
          {activeTab === "smart" ? (
            <>
              ...
              {displayList.length > 0 ? (
                displayList.map((match) => (
                  <MatchCard key={match.id} match={match} variant="smart" />
                ))
              ) : (
                <div className="col-span-full text-center py-20">
                  <p className="text-white/40">לא נמצאו התאמות.</p>
            ...
            offers.map((offer) => (
              <MatchCard key={offer.id} match={offer} variant="offer" />
            ))
          )}
        </div>
      </div>
