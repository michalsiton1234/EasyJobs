...
// @ts-nocheck
import { useCandidateOffers } from "@/hooks/useCandidateOffers";
import OfferCard from "@/components1/candidate/OfferCard";
import "@/style/candidate/CandidateOffers.css";

export default function CandidateOffers() {
  const { matches, loading, handleRespond } = useCandidateOffers();

  if (loading) return <div className="offers-loading">טוען הצעות...</div>;

  return (
    <div className="offers-page" dir="rtl">
      <h1 className="offers-title">הצעות העבודה שלי</h1>

      <div className="offers-grid">
        {matches.length === 0 ? (
          <p className="offers-empty">אין הצעות חדשות כרגע.</p>
        ) : (
          matches.map((match) => (
            <OfferCard
              key={match.MatchId}
