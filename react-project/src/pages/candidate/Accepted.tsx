...
import AcceptedTabs from "@/components1/candidate/AcceptedTabs";
...
export default function Accepted() {
  ...
  if (loading) {
    return (
      <div className="accepted-loader">
        <div className="accepted-spinner" />
      </div>
    );
  }

  return (
    <div className="accepted-page" dir="rtl">
      <div className="accepted-wrapper">
        ...
        <motion.div
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          className="accepted-header"
        >
          <button
            onClick={() => navigate("/candidate/my-area")}
            className="accepted-back-btn"
          >
            <span>חזרה לאזור אישי</span>
            <ArrowLeft className="w-4 h-4" />
          </button>

          <div className="accepted-header-right">
            <div className="accepted-header-text">
              <h1>התאמות שלי, {getUserName() || user?.full_name || "משתמש"}</h1>
              <p>כל ההתאמה שעברו אלגוריתם</p>
            </div>
            <div className="accepted-avatar">🎯</div>
          </div>
        </motion.div>
